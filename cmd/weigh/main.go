...
func main() {
	...
	shutdownTracer, err := ztrace.InstallOpenTelemetryTracer(&cfg.Tracing, logger, appName, Version)
	if err != nil {
	...
	a, err := app.New(*cfg)
	if err != nil {
		_ = level.Error(logger).Log("msg", "failed to create "+appName, "err", err)
		os.Exit(1)
	}

	if err := a.Run(); err != nil {
		_ = level.Error(logger).Log("msg", "error running "+appName, "err", err)
		os.Exit(1)
	}
}
